...
# Add column names
attribute_list =  [f"pixel_{i+1}" for i in range(df.shape[1])]
df.columns = attribute_list

# Add target variable as a new column
df['Class'] = digits.target


#Dataset pre-processing and creating k-folds k is 10
split_dataset = []
ratios = []
dataset_length = df.shape[0]
unique_target_vals = df.iloc[:,-1].unique()
for vals in unique_target_vals:
    rows = df.loc[df['Class'] == vals]
...
def _info_gain(df,attribute):
    dataset_entropy = _entropy(df.iloc[:,0])
...
def k_fold_cross_validation(k_fold_datasets,ntree):
    ...
    for i in range(len(k_fold_datasets)):
        ...
        #print("prediction",each_fold_predictions)
